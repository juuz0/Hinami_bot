 ...
 client.on('message', msg => { 
  ...
  msg.delete(); }
	...
	if(userlist.size>0){msg.channel.send("Please don't mention someone");}     
	 ...
	 if(cmd==='dict'){
		 webdict('urbandictionary',msg.content.slice(1)).then(response=>{if(response.statusCode==200){msg.channel.send({embed:new Discord.RichEmbed().setTitle(args[0]).setDescription(response.definition[0])})} else{msg.reply("Please check the spelling again")}});
		 }
	 if(cmd==='invite'){
